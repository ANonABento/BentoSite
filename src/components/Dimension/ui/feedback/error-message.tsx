...
export function ErrorMessage({ error, onRetry, isMobile }: ErrorMessageProps) {
  ...
  const getErrorIcon = () => {
    switch (error.code) {
      case 'FILE_NOT_FOUND':
        return (
          <svg className="w-16 h-16 text-red-400 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.732 16.5c-.77.833.192 2.5 1.732 2.5z" />
          </svg>
        );
      case 'INVALID_FORMAT':
        return (
          <svg className="w-16 h-16 text-orange-400 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
      ...
      <div className={`${DESIGN_SYSTEM.colors.bg.primary} backdrop-blur-sm rounded-xl p-8 text-center border ${DESIGN_SYSTEM.colors.border.secondary} pointer-events-auto shadow-2xl ${isInitialRender ? '' : 'transform transition-all duration-200 hover:scale-105'} ${isMobile ? 'mx-6 max-w-sm' : 'max-w-lg mx-4'}`}>
        ...
        <div className="space-y-4">
          {error.retryable && (
            <button 
              onClick={onRetry} 
              className={`w-full bg-red-600 hover:bg-red-700 text-white font-medium py-3 px-6 rounded-xl ${isInitialRender ? '' : 'transition-all duration-200 flex items-center justify-center space-x-3 transform hover:scale-105'} focus:outline-none focus:ring-2 focus:ring-red-500/50 shadow-lg hover:shadow-xl`}
            >

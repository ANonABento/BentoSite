...
The info card's position tracking is broken. When the user pans away from the
info card and then pans back, they have to pan the EXACT same distance back
before the info card decompresses. This makes the sticky behavior feel broken.

### Root Cause

The info card has a fixed "home" position in canvas space (its grid cell near
...
1. User pans right → info card hits left edge → compresses to icon strip
2. User continues panning right → info card stays as icon strip at left edge
3. User pans back LEFT a small amount (50-100px) → info card immediately
   starts decompressing
4. User pans back a bit more → info card fully decompresses to full card
...
### 3. Making info card track viewport center
Made the info card always centered — it never compresses because it's always
...
Produced extreme canvas values (x=2000+) that made the info card's "position"
meaningless for spawning and grid calculations.

## Correct Solution

The info card needs TWO position concepts:
...
**The info card should render as `position: fixed` ALWAYS** (not in the canvas
...
So the fix is:
- **Revert the info card to `position: fixed` overlay** (like pre-Phase A)
...
The key insight that was missing: the info card being `position: fixed` doesn't
...
1. `DesktopCanvasView.tsx`: Render InfoMenuCard as a sibling of the canvas
   layer (not inside it), with `position: fixed`. Pass screen-space position
   from useInfoCardState.

2. `DesktopCardLayer.tsx`: Skip rendering the info card (it's rendered
   separately). When encountering INFO_CARD_ID in layouts, return null.

3. `useInfoCardState.ts`: Revert to computing position from canvasToScreen
...
4. `InfoMenuCard.tsx`: Change back to `positionMode="fixed"`.
...
- The info card renders at the clamped screen position (e.g., left edge)
...
OR: track the info card's position as a **spring** that follows the
viewport center with lag. When the camera moves, the info card's tracked
...
```
// Each frame:
searchTarget = viewportCenter  // where the info card wants to be
...
This gives:
- Pan right → info card lags behind → hits left edge → sticks → compresses
- Keep panning → info card stays at left edge, fully compressed
- Pan back → info card's spring target moves back → immediately pulls away
  from edge → starts decompressing
- Keep panning back → info card fully decompresses and centers
...
| File | Change |
|------|--------|
| `views/DesktopCanvasView.tsx` | Render info card as fixed overlay again, pass tracked position |
| `views/DesktopCardLayer.tsx` | Skip INFO_CARD_ID, don't render in canvas layer |
| `cards/useInfoCardState.ts` | Implement spring-follow position tracking with edge clamping |
| `cards/InfoMenuCard.tsx` | positionMode="fixed", keep icon strip/compact bar |
...
ONLY about how the info card's render position is computed and where it
renders in the DOM.

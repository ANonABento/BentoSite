...
When the info card compresses to a viewport edge and the user pans back,
the info card tries to return to its original grid home (e.g., near canvas
origin). If the user panned 2000px away, the info card won't fully
decompress until they pan all 2000px back.

## Solution: Re-home to nearest viewport gap

When the user pans back and the info card starts decompressing, it should
...
1. User pans right → info card hits left edge → compresses → sticks
...
4. Info card's grid position is updated to that cell
5. Compression is now relative to the new nearby home → decompresses quickly
6. Info card fully decompresses into the new grid cell

### Implementation

**`useBoardController.ts`** — add `rehomeInfoCard(camera, windowSize)`:
- Computes viewport center in grid coordinates
- Finds nearest available cell via `grid.findNearest(centerCol, centerRow, '2x1')`
- Releases old info card cells, places at new cell
- Updates `visible` map with new position

**`DesktopCanvasView.tsx`** — call `rehomeInfoCard` when compression drops
...
- The info card's `searchCardLayout` in `board.visible` updates
- `useInfoCardState` recomputes from the new (nearby) position
- Decompression completes quickly since the new home is close

### Key constraint

Only rehome when the info card is decompressing (panning back toward it).
...
| File | Change |
|------|--------|
| `core/useBoardController.ts` | Add `rehomeInfoCard` method |
| `views/DesktopCanvasView.tsx` | Call rehome on decompression transition |
| `docs/bentogrid-handoff.md` | Update info card behavior section |
